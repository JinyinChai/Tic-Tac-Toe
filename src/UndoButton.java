...
public class UndoButton extends JButton implements Observer {
	...
	public UndoButton(Board aModel)  {
		...
		this.addActionListener(new ActionListener() {  //Adds an ActionListener for the button
			public void actionPerformed(ActionEvent e) {
				...
				newTurn(previousModelSize);  //Checks if it's a new turn, it's a new turn if the number of elements in the model is larger than it was before the button was pressed
	...
	public void eligibility() {
		...
		else if (!counter.isEmpty() && counter.peek() % 3 == 0) {  //Disables the button if it has been pressed three times in a single turn
			this.setEnabled(false);
	...
	public boolean newTurn(int position) {
		if (modelSize > position) {  //Compares the the number of elements in the board  to determine if it's a new turn
			resetCounter();  //Resets the stack
			return true;
		}
		return false;
